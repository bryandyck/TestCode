...
namespace WindowsApplication3
{
    ...
    public class NormalContinuousDistribution 
    {
        ...
        // Methods
        static NormalContinuousDistribution()
        {
        ...
        public NormalContinuousDistribution()
            : this((double)0, (double)1)
        {
        }

        public NormalContinuousDistribution(double mean, double var)
        {
            this.mean_ = mean;
            this.Variance = var;
        }

        public NormalContinuousDistribution(double[] _values)
        {
            SetValues( _values );
        }
        ...
        public void SetValues(double[] values)
        {
            ...
            var = 0;
            for (i = 0; i < values_.GetUpperBound(0) + 1; i++)
                var = var + Math.Pow((values_[i] - mean), 2d);
            var = (var / (values_.GetUpperBound(0)));
        ...
        public double Kurtosis 
        {
            get
            {
                ...
                sumTo4 = 0;
                for(i = 0; i < n; i++)
                    sumTo4 = sumTo4 + Math.Pow(((values_[i] - mean_) / sigma_), 4d);
        ...
        { 
            get
            {
                ...
                sumTo3 = 0;
                for(i = 0; i<n; i++)
                    sumTo3 = sumTo3 + Math.Pow(((values_[i] - mean_) / sigma_), 3);
